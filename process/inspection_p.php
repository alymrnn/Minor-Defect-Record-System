...
// FETCHING CAR MAKER AND MODEL
if ($method == 'get_inspection_details') {
    $ip_address = $_GET['ip_address'];

    // Check if the IP address exists in m_inspection_ip
    $query1 = "SELECT ircs_line FROM m_inspection_ip WHERE ip_address = ?";
    ...
    if ($row1) {
        ...
        $query2 = "SELECT car_maker, car_model FROM m_ircs_line WHERE ircs_line = ?";
        ...
        if ($row2) {
            ...
            echo json_encode(
                array(
                    'error' => 'No matching car maker and car model found in m_ircs_line for the given IRCS line'
                )
            );
        }
    } else {
